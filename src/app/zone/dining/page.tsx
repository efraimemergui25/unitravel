...
import { DiningControl }      from '@/components/zones/DiningControl';
import { CulinaryFilters }    from '@/components/zones/CulinaryFilters';
...
// Pre-configured for Effi & Nofar honeymoon context
const DEFAULT_VIBES = ['honeymoon-romantic'];
const DEFAULT_DIETS = ['omnivore'];

const QUERY = {
  destination: 'Tulum · CDMX · Los Cabos · Miami',
...
export default function DiningPage() {
  ...
  const [selectedVibes, setSelectedVibes] = useState<string[]>(DEFAULT_VIBES);
  const [selectedDiets, setSelectedDiets] = useState<string[]>(DEFAULT_DIETS);
  ...
  const handleSearch = useCallback((engineIds: string[]) => {
    ...
    // Dining APIs are async-heavy — 2.6s scan
    setTimeout(() => {
  ...
  return (
    <div
      style={{
        ...
        overflow:   'hidden',
        background: [
          `radial-gradient(ellipse at 0% 0%, ${COLOR}0B 0%, transparent 52%)`,
          'radial-gradient(ellipse at 100% 100%, rgba(255,69,58,0.04) 0%, transparent 48%)',
          '#F2F2F7',
        ].join(', '),
      }}
    >
      {/* ── Left: Dining Control Console ──────────────────── */}
      <DiningControl onSearch={handleSearch} isSearching={searchState === 'loading'} />
      ...
      >
        ...
        >
          <DestTag label={QUERY.destination} />
        ...
        >
          <DiningBento searchState={searchState} engineCount={engineCount} />
        </div>
      </div>
    </div>
  );
...
function ScanProgressPill({ progress, engineCount }: { progress: number; engineCount: number }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 148 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: COLOR, letterSpacing: '-0.01em' }}>
          Merging {engineCount} culinary sources
        </span>
        <div style={{ height: 3, borderRadius: 999, background: `${COLOR}1C`, overflow: 'hidden' }}>
          <motion.div
            ...
            style={{
              height:       '100%',
              background:   `linear-gradient(90deg, ${COLOR}, #FF453A)`,
              borderRadius: 999,
            }}
          />
        </div>
