...
// Tier-1 AI picks — top-quality experiences platforms
const AI_PICKS = new Set([
  'tripadvisor-a', 'google-places', 'viator', 'getyourguide', 'airbnb-exp',
  'klook', 'musement', 'atlas-obscura', 'culture-trip',
]);

const ENGINES = ZONE_ENGINES['attractions'];

const TIER_LABELS: Record<1 | 2 | 3, string> = {
  1: '⭐ Top Booking Platforms',
...
function ModeToggle({ mode, onChange }: { mode: Mode; onChange: (m: Mode) => void }) {
  return (
    <LayoutGroup id="attractions-mode">
      <div
        role="group"
        ...
        {(['ai', 'god'] as Mode[]).map(m => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="attractions-mode-pill"
                  style={{
...
export interface AttractionsControlProps {
...
export function AttractionsControl({ onSearch, isSearching }: AttractionsControlProps) {
    ...
    >
      ...
      <div style={{ padding: '22px 18px 14px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: 'var(--text-tertiary)', fontWeight: 500, letterSpacing: '-0.01em' }}>
              {ENGINES.length} experience networks
            </div>
          </div>
        ...
        <p style={{
          ...
          Weather-synced · Effort-filtered · Spatially ranked by proximity
        </p>
        ...
        >
          {isSearching ? (
            ...
            <>
              ...
              {count === 0 ? 'Select engines' : `Search ${count} experience engine${count !== 1 ? 's' : ''}`}
            </>
          )}
        </motion.button>
      </div>
