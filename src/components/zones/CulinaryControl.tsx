...
function ModeToggle({ mode, onChange }: { mode: Mode; onChange: (m: Mode) => void }) {
  return (
    <LayoutGroup id="culin-mode-strip">
      <div
        role="group"
        ...
        {(['ai', 'god'] as Mode[]).map(m => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="culin-mode-pill-strip"
                  style={{
                    position:     'absolute',
                    inset:        0,
                    borderRadius: 7,
...
const GlassPill = memo(function GlassPill({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.94 }}
      animate={isActive ? {
        boxShadow: 'inset 0 0 10px rgba(255,255,255,0.8), 0 4px 14px rgba(255,159,10,0.18)',
      } : {
...
// ── CulinaryControl — horizontal strip ───────────────────────────────────────

export interface CulinaryControlProps {
  onSearch:    (engineIds: string[]) => void;
  isSearching: boolean;
}

export function CulinaryControl({ onSearch, isSearching }: CulinaryControlProps) {
  ...
  const handleModeChange = useCallback((m: Mode) => {
    ...
    else setSelected(new Set(ENGINES.map(e => e.id)));
  }, []);
    ...
    >
      ...
      >
        ...
        >
          ...
          {isSearching ? `Scanning ${count}…` : `Search ${count} restaurants`}
        </motion.button>
      </div>
      ...
      <div
        ...
        aria-label="Dining engine selector"
      >
        {ENGINES.map(engine => (
          <GlassPill
            key={engine.id}
      ...
      <AnimatePresence mode="wait">
        {mode === 'ai' && (
          ...
          >
            ✦ AI optimized: {AI_PICKS.size} top-trust culinary engines selected.
          </motion.p>
        )}
      </AnimatePresence>
    </motion.div>
